  ...
  container.innerHTML = `
    <div class="screen">
      ...
      <div class="sticky-footer" style="display:flex; flex-direction:column; gap:8px;">
        <button class="btn btn-primary btn-full" id="confirm-orders-btn" onclick="confirmOrders()" style="margin-bottom: 0;">
          ✅ Confirm All Orders
        </button>
        <div style="display:flex; gap:8px;">
          <button class="btn btn-ghost" onclick="exportOrdersToCSV()" style="flex:1; padding:10px; font-size:13px; display:flex; align-items:center; justify-content:center; gap:6px;">
            📥 Export CSV
          </button>
          <button class="btn btn-ghost" onclick="shareOrdersOnWhatsApp()" style="flex:1; padding:10px; font-size:13px; display:flex; align-items:center; justify-content:center; gap:6px;">
            💬 WhatsApp Share
          </button>
        </div>
      </div>
...
// ── Export and Share Order List ───────────────────────────────────────────────
function exportOrdersToCSV() {
  if (!_currentRecs || _currentRecs.length === 0) {
    showToast('No orders to export', 'error');
    return;
  }
  const headers = ['Item Name', 'Category', 'Quantity', 'Original AI Recommendation', 'Signal/Reason'];
  const rows = _currentRecs.map(item => {
    const finalQty = _modifiedQtys[item.item_name] ?? item.recommended_qty;
    return [
  ...
  link.setAttribute('download', `hotel_aditya_orders_${_currentRecDate || 'export'}.csv`);
...
function shareOrdersOnWhatsApp() {
  if (!_currentRecs || _currentRecs.length === 0) {
    showToast('No orders to share', 'error');
    return;
  }

  const dateFmt = _currentRecDate ? fmtDate(_currentRecDate) : 'Tomorrow';
  let message = `🏨 *Hotel Aditya Grand - Orders for ${dateFmt}*\n\n`;

  const CAT_ORDER = ['biryani','chicken','bread','dairy','rice','beverage','starter','seafood','ice_cream','family_pack','egg','soup','other'];
  const grouped = {};
  _currentRecs.forEach(rec => {
    const qty = _modifiedQtys[rec.item_name] ?? rec.recommended_qty;
    if (qty <= 0) return; // Only list items with non-zero qty
    const cat = rec.category || 'other';
    if (!grouped[cat]) grouped[cat] = [];
    grouped[cat].push({ name: rec.item_name, qty });
  });

  let hasItems = false;
  for (const cat of CAT_ORDER) {
    if (!grouped[cat] || grouped[cat].length === 0) continue;
    const catLabel = CATEGORY_META[cat]?.label || cat.toUpperCase();
    message += `*${catLabel}*\n`;
  ...
  // Any remaining categories
  for (const cat of Object.keys(grouped)) {
    if (CAT_ORDER.includes(cat) || grouped[cat].length === 0) continue;
  ...
  if (!hasItems) {
    showToast('All order quantities are zero', 'error');
    return;
  }

  message += `_Generated by Aditya Grand AI Order Assistant_`;
